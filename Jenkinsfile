    ...
    stages {
        ...
        stage("Build IDL files") {
            steps {
                script {
                    sh """
                    docker exec -u saluser \${container_name} sh -c \"source ~/.setup.sh && setup ts_sal -t current && make_idl_files.py ATMonochromator\"
                    """
                }
            }
        }
        stage("Install config_atcalsys") {
            steps {
                script {
                    sh """
                    docker exec -u saluser \${container_name} sh -c \"source ~/.setup.sh && cd /home/saluser/repos/ && git clone https://github.com/lsst-ts/ts_config_atcalsys.git && cd /home/saluser/repos/ts_config_atcalsys/ && /home/saluser/.checkout_repo.sh \${work_branches} && git pull && eups declare -r . -t saluser\"
                    """
                }
            }
        }
        stage("Running tests") {
            steps {
                script {
                    sh """
                    docker exec -u saluser \${container_name} sh -c \"source ~/.setup.sh && cd repo && eups declare -r . -t saluser && setup ts_monochromator -t saluser && export LSST_DDS_IP=192.168.0.1 && pytest --color=no -ra --junitxml=tests/results/results.xml\"
                    """
                }
            }
        }
